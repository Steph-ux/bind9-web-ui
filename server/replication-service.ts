...
class ReplicationService {
  ...
  private async syncToServer(server: ReplicationServer, zones: any[]): Promise<SyncResult> {
    ...
    try {
      ...
      try {
        ...
        try {
          const slaveConfig = await this.generateSlaveConfig(zones, server);
      ...
      try {
        ...
        try {
          for (const zone of masterZones) {
            if (!/^[a-zA-Z0-9._-]+$/.test(zone.domain)) continue; // Skip invalid domains
        ...
        // Cannot connect to server â€” skip conflict detection for this server
  ...
  private async generateSlaveConfig(zones: any[], server: ReplicationServer): Promise<string> {
    const lines: string[] = ["// Auto-generated by BIND9 Web Manager â€” DO NOT EDIT MANUALLY"];
    // Resolve the master's IP from the active SSH connection
    let masterIp = "127.0.0.1";
    try {
      const activeConn = await storage.getActiveConnection();
      if (activeConn) masterIp = activeConn.host;
    } catch {}
